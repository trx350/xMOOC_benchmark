/**
	** Modifications to the original webgazer.js library **
	
	The folowing modifications (overwrites) are introduced:
	 - added var localstream = null; to capture initiated stream to finish it;
	 - function webgazer.endAndStop and webgazer.endAndStopNoStore to end running webgazer;
	 - function webgazer.localstream to handle initiated stream;
	 - clickListener;
	 - clickAutoCalibrateListener;
...
(function(window, undefined) {
    ...
    //var eventTypes = ['click', 'move','autoclick', 'automove'];
	...
	var clickAutoCalibrateListener = function(event) {
        recordScreenPosition(event.clientX, event.clientY, eventTypes[0]); // eventType[0] === 'click'
    }
    ...
    var moveListener = function(event) {
		//console.log("move listener in gazer");
        if (paused) {
            return;
        }
	   ...
	   var moveAutoCalibrateListener = function(event) {
		  // console.log("automove listener in gazer");
        if (paused) {
            return;
        }
    ...
    var addMouseEventListeners = function() {
		...
		//document.addEventListener('autocalibratewg', clickAutoCalibrateListener, true);
		//document.addEventListener('autoclick', clickAutoCalibrateListener, true);
        document.addEventListener('mousemove', moveListener, true);
		//document.addEventListener('automove', moveAutoCalibrateListener, true);
    };
    ...
    */
	...
	/**
	 * Stops webgazer without the store option
     */	 
	webgazer.endAndStopNoStore = function() {
        //loop may run an extra time and fail due to removed elements
        paused = true;
        ...
        try{
	        document.body.removeChild(videoElement);
	        document.body.removeChild(videoElementCanvas);
			if(document.getElementById('overlay')) {			// if overlay used, remove it as well
				var item = document.getElementById("overlay");
        ...
        // setGlobalData();  // --- do not store anything
        return webgazer;
    }
	
	/**
	 * Stops webgazer with the store option (setGlobalData())
     */	 
	webgazer.endAndStop = function() {
        //loop may run an extra time and fail due to removed elements
        paused = true;
		localstream.getTracks()[0].stop();
        //remove video element and canvas
